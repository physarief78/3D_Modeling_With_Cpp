...
// Function to generate points for a torus-shaped coil with y-axis separated
std::vector<Point3D> generateTorus_yaxis(double majorRadius, double minorRadius, double centerX, double centerY, double centerZ, int numMajorPoints, int numMinorPoints) {
    std::vector<Point3D> coilPoints;
    double majorAngleStep = 2 * M_PI / numMajorPoints;
    double minorAngleStep = 2 * M_PI / numMinorPoints;
...
void saveCoilGeometryToFile(const std::string& filename, const std::vector<Point3D>& coil1, 
    const std::vector<Point3D>& coil2, const std::vector<Point3D>& coil3, const std::vector<Point3D>& coil4) {
...
int main() {
    ...
    double currentCoil3 = 1.0;
    double currentCoil4 = -1.0;
    ...
    // Generate points for the third torus
    std::vector<Point3D> coil3 = generateTorus_yaxis(majorRadius, minorRadius, 0.0, separation / 2.0, 0.0, numMajorPoints, numMinorPoints);

    // Gnerate points for the fourth torus
    std::vector<Point3D> coil4 = generateTorus_yaxis(majorRadius, minorRadius, 0.0, - separation / 2.0, 0.0, numMajorPoints, numMinorPoints);

    // Save coil geometry
    saveCoilGeometryToFile("two_coil_pair_geometry.txt", coil1, coil2, coil3, coil4);

    // Define a grid of points to calculate the magnetic field
    std::vector<std::pair<Point3D, Vector3D>> fieldData;
    double gridRange = 12.0; // Range of the grid (from -gridRange to +gridRange)
    ...
    for (double x = -gridRange; x <= gridRange; x += gridStep) {
        for (double y = -gridRange; y <= gridRange; y += gridStep) {
            for (double z = -gridRange; z <= gridRange; z += gridStep) {
                ...
                Vector3D B3 = calculateMagneticField(p, coil3, currentCoil3);
                Vector3D B4 = calculateMagneticField(p, coil4, currentCoil4);

                // Total magnetic field is the vector sum of contributions
                Vector3D B = B1 + B2 + B3 + B4;
    ...
    saveFieldToFile("two_coil_pair_magnetic_field_data.txt", fieldData);
